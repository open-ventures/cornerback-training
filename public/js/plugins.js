		...
		menuHandler = function(){
			jQuery('.l-header .w-nav').each(function () {
				...
				if (windowWidth < 1000) {
					...
					navList.css({height: '0', display: 'none'});
					navSubLists.css({height: '0', display: 'none'});

					navControl.off('click').click(function() {
						if (listOpen) {
							navList.animate({height: 0}, 250, function(){
								navList.css({display: 'none'});
								navList.find('.w-nav-list').css({display: 'none', height: 0}).data('subListOpen', false);
							});
							listOpen = false;
						} else {
							if (!listHeight) {
								navList.css({opacity: 0, display: 'block', height: ''});
					...
					navSubControls.off('click').click(function() {
						...
						if (subListOpen) {
							subList.animate({height: 0}, 250, function(){
								subList.css({display: 'none'});
								subList.find('.w-nav-list').css({display: 'none', height: 0}).data('subListOpen', false);
							});
							subListOpen = false;
						} else {
							if (!subListHeight) {
								subList.css({opacity: 0, display: 'block', height: ''});
								subListHeight = subList.height();
								subList.css({opacity: 1});
							}
							subList.css({height: 0, display: 'block'});
							subList.animate({height: subListHeight}, 250, function(){
								subList.css({height: ''});
							});
							subListOpen = true;
						}
